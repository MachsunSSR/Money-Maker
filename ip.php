...
if ($pilih == 1) {
   echo "\nMasukkan nama file dengan list proxy (contoh: cek.txt): ";
   $nama_file = trim(fgets(STDIN));
   if(file_exists($nama_file)) {
       $ambil = explode(PHP_EOL, file_get_contents($nama_file));
...
function cekip($proxy)
{
   ...
   if ($resp == NULL) {
       ...
       print_r("$proxy : [ LIVE ] IP : $ip | Lokasi : $region | Kode Negara : $country | Kode Pos : $zip".PHP_EOL);
       file_put_contents('live_proxy.txt', $proxy.PHP_EOL, FILE_APPEND);
   } else {
       print_r("$proxy : [ ERROR ]\n");
   }
}
